...
module.exports.run = async(client, message, args) => {
    ...
    switch(args[0]) {
        case `cash`: lb.sort((a, b) => (a.balance <= b.balance) ? 1: -1); break;
        case `kills`: lb.sort((a, b) => (a.kills <= b.kills) ? 1: -1); break;
        case `xp`: lb.sort((a, b) => (a.experience <= b.experience) ? 1: -1); break;
        case `tech`: lb.sort((a, b) => (a.tech <= b.tech) ? 1: -1); break;
        default:
    ...
    for(let i = 0; i < 25; i++) {
        switch(args[0]) {
            case `cash`: lbTxt += `${i == 0 ? `🥇`: i == 1 ? `🥈`: i == 2 ? `🥉`: `🏅`} - ${lb[i].username} - ${lb[i].balance}\n`; break;
            case `kills`: lbTxt += `${i == 0 ? `🥇`: i == 1 ? `🥈`: i == 2 ? `🥉`: `🏅`} - ${lb[i].username} - ${lb[i].kills}\n`; break;
            case `xp`: lbTxt += `${i == 0 ? `🥇`: i == 1 ? `🥈`: i == 2 ? `🥉`: `🏅`} - ${lb[i].username} - ${lb[i].experience}\n`; break;
            case `tech`: lbTxt += `${i == 0 ? `🥇`: i == 1 ? `🥈`: i == 2 ? `🥉`: `🏅`} - ${lb[i].username} - ${lb[i].tech}\n`; break;
        }
    }

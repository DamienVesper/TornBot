...
request(`https://torn.space/leaderboard`, (err, res, body) => {
    ...
    for(let i in rawTornUsers) {
        ...
        if(rawUO[1].slice(0, 1) == `[`) {
            ...
            if(tagEnd == 2) {
                switch(rawUO[1].slice(1, 2)) {
                    case `V`: typeUO = `VIP`; break;
                    case `M`: typeUO = `Moderator`; break;
                    case `A`: typeUO = `Administrator`; break;
                    case `O`: typeUO = `Owner`; break;
                }
            }
            else typeUO = `Player`;
        ...
        tornUsers[newUO] = { displayName: rawUO[1] };

        switch(rawUO[0].slice(13, 17)) {
            case `cyan`: tornUsers[newUO].team = `Human`; break;
            case `pink`: tornUsers[newUO].team = `Alien`; break;
            case `lime`: tornUsers[newUO].team = `Green`; break;
        }
    ...
    return module.exports = { tornUsers };
});
